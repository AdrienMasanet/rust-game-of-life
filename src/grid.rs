...
impl Grid {
    ...
    pub fn populate_cells(&mut self) {
        ...
        while cells_left_to_make_alive > 0 {
            ...
            // Get the cell at the random position that was generated by getting the value of the key (x, y)
            let mut cell_to_make_alive: &mut Cell = self.cells.get_mut(&(x, y)).unwrap();

            // Check if the cell is already alive in which case we don't want to make it alive again and mislead the cells_left_to_make_alive counter
    ...
    pub fn update(&mut self) {
        ...
        for x in 0..self.cells_width {
            for y in 0..self.cells_height {
                ...
                let mut cell: &mut Cell = self.cells.get_mut(&(x, y)).unwrap(); // We borrow the cell again because self was re-borrowed in the count_alive_neighbors function
    ...
    fn count_alive_neighbors(&self, cell_x: i32, cell_y: i32) -> u8 {
        ...
        // Check the neighbors of the cell in the 8 directions
        for x in -1..=1 {
            for y in -1..=1 {
                // Check if the neighbor is the cell itself in which case we skip it
                if x == 0 && y == 0 {
                    continue;
                }
